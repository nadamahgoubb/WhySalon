...
/**
 * A utility object to centralize the logic of converting a whysalonResponse] to a whysalonNetworkResponse]
 */
internal object ResponseHandler {

    /**
     * Converts the given whysalonresponse] to a subclass of whysalonNetworkResponse] based on different conditions.
     *
     * If the server response is successful with:
     * => a non-empty body -> NetworkResponse.Success<S, E>
     * => an empty body (and whysalonsuccessBodyType] is Unit) -> NetworkResponse.Success<Unit, E>
     * => an empty body (and whysalonsuccessBodyType] is not Unit) -> NetworkResponse.ServerError<E>
     *
     * @param response Retrofit's response object supplied to the call adapter
     * @param successBodyType A whysalonType] representing the success body
