    ...
    private fun handleViewState(action: AuthAction) {
        when (action) {
            ...
            is AuthAction.OtpChecked -> {
                showProgress(false)
                if (action.data.exists == true) state3()
                else showToast(resources.getString(R.string.wrong_otp))
    ...
    private fun onClick() {
        ...
        binding.btnDone.setOnClickListener {
            if (!binding.etOtp.otp.isNullOrBlank()) {
                ...
                mViewModel.email?.let { it1 -> mViewModel.checkOtp(it1, binding.etOtp.otp!!) }
            } else {
                showToast(resources.getString(R.string.empty_msg_otp))
            }
        }
