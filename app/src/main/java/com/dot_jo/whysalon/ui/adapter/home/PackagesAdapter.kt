...
if(list.size>1) {
    ...
    override fun onBindViewHolder(holder: PackagesViewHolder, position: Int) {
        ...
        var currentItem = list[position]
         holder.binding.tvMoney.setText(context.resources.getString(R.string.price_from)+currentItem?.price?.toDoubleOrNull()?.roundToInt()+context.resources.getString(
            R.string.sr))
         holder.binding.tvServicesType.setText(currentItem.name)
