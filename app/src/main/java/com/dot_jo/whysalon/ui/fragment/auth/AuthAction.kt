...
sealed class AuthAction() : Action {
    ...
    data class ShowFailureMsg(val message: String?) : AuthAction()
     ...
     data  class RegisterSucess(val data: LoginResponse, val social: Boolean) : AuthAction()
     data  class ContinueAsGuest(val data: LoginResponse) : AuthAction()
    data  class LoginSuccess(val data: LoginResponse, val social: Boolean) : AuthAction()
